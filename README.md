...
**Status:** MVP demo is **showcase-ready** for local handoff — creator flow, profile eligibility, public vote/results, and integrated manual QA are documented and guarded. **Phase 70 route adapters** now resolve production identity through `UserAuthResolver`; **Phase 71** documents formal session lifecycle planning; **Phase 72** adds the minimal production credential verifier foundation (`USER_AUTH_CREDENTIALS_JSON`) while keeping missing config fail-closed; **Phase 73** documents production login/session UX planning only; **Phase 77** recommends HttpOnly Secure SameSite cookie session as the next production browser login/session runtime while preserving Bearer as a controlled verifier path; **Phase 78** adds only the digest-backed `user_sessions` schema/repository foundation. This is still **not** production-deploy-ready: there is **no** production login UX implementation yet; no session cookie is issued; local/demo visitors still use **MVP demo-style `X-User-Id`** and creators use **`creator_session`** (non-production identity). **Production user-auth wiring later** still requires frontend login/session UX implementation and a separately approved implementation phase to wire `user_sessions` into the production verifier. See **`docs/www-project-phase-78-production-session-schema-foundation-v1.md`**, **`docs/www-project-phase-77-production-login-session-runtime-plan-v1.md`**, **`docs/www-project-phase-73-production-login-session-ux-plan-v1.md`**, **`docs/www-project-phase-72-production-credential-verifier-foundation-v1.md`**, **`docs/www-project-phase-71-production-credential-verifier-plan-v1.md`**, and **`docs/www-project-phase-70-final-production-auth-boundary-checkpoint-v1.md`**.
...
The public browser surface remains **share-link first**. Default routes stay **static/demo-facing** for policy UX; **creator lifecycle management** is available only with MVP dev query switches (`?live=1`, `?creator=1`) and the Phase 65A **`creator_session` cookie** (local/demo/test only — **not** production public identity). Backend route adapters for profile, Official Vote, vote-by-index, and creator-owned polls use **`UserAuthResolver`** in production mode; the browser still sends **MVP demo-style `X-User-Id`** until frontend production login is implemented. Results follow backend `public_lifecycle_state`. Phase 72 adds a minimal production credential verifier foundation, Phase 73 documents production login/session UX planning, Phase 77 recommends HttpOnly Secure SameSite cookie session as the next production browser runtime, and Phase 78 adds only the digest-backed `user_sessions` schema/repository foundation, but there is still **no** production login UX implementation, session cookie issuance, notification persistence, trust scoring persistence, feedback persistence, or production ranking/feed personalization. Auth boundary summary: **`docs/www-project-phase-70-final-production-auth-boundary-checkpoint-v1.md`**. Production credential verifier and login/session planning/foundation: **`docs/www-project-phase-71-production-credential-verifier-plan-v1.md`**, **`docs/www-project-phase-72-production-credential-verifier-foundation-v1.md`**, **`docs/www-project-phase-73-production-login-session-ux-plan-v1.md`**, **`docs/www-project-phase-77-production-login-session-runtime-plan-v1.md`**, and **`docs/www-project-phase-78-production-session-schema-foundation-v1.md`**.
