...
**Status:** MVP demo is **showcase-ready** for local handoff — creator flow, profile eligibility, public vote/results, and integrated manual QA are documented and guarded. **Phase 70 route adapters** now resolve production identity through `UserAuthResolver`; **Phase 71** documents formal session lifecycle planning; **Phase 72** adds the minimal production credential verifier foundation (`USER_AUTH_CREDENTIALS_JSON`) while keeping missing config fail-closed. This is still **not** production-deploy-ready: there is **no** production login UX yet; local/demo visitors still use **MVP demo-style `X-User-Id`** and creators use **`creator_session`** (non-production identity). **Production user-auth wiring later** still requires frontend login/session UX. See **`docs/www-project-phase-72-production-credential-verifier-foundation-v1.md`**, **`docs/www-project-phase-71-production-credential-verifier-plan-v1.md`**, and **`docs/www-project-phase-70-final-production-auth-boundary-checkpoint-v1.md`**.
...
The public browser surface remains **share-link first**. Default routes stay **static/demo-facing** for policy UX; **creator lifecycle management** is available only with MVP dev query switches (`?live=1`, `?creator=1`) and the Phase 65A **`creator_session` cookie** (local/demo/test only — **not** production public identity). Backend route adapters for profile, Official Vote, vote-by-index, and creator-owned polls use **`UserAuthResolver`** in production mode; the browser still sends **MVP demo-style `X-User-Id`** until frontend production login is implemented. Results follow backend `public_lifecycle_state`. Phase 72 adds a minimal production credential verifier foundation, but there is still **no** production login UX, notification persistence, trust scoring persistence, feedback persistence, or production ranking/feed personalization. Auth boundary summary: **`docs/www-project-phase-70-final-production-auth-boundary-checkpoint-v1.md`**. Production credential verifier planning/foundation: **`docs/www-project-phase-71-production-credential-verifier-plan-v1.md`** and **`docs/www-project-phase-72-production-credential-verifier-foundation-v1.md`**.
