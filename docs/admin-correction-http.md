...
**Integration validation (Phase 10):** `tests/integration/admin-audit-http.pg.test.ts` now covers the Phase 8 public notice read API and Phase 9 global audit queue against PostgreSQL repositories, including safe DTO boundaries, empty/hidden notice cases, fixed keyset ordering, bounded pagination, filters, admin guards, and rejected client sorting. `npm run test:integration` requires `DATABASE_URL` on an isolated test database (e.g. `www_test`). It was **not executed** during Phase 10 synchronization because `DATABASE_URL` was missing. This is an environment gap, not a Phase 10 code failure. Unit tests (`npm test`) cover HTTP and service contracts without PostgreSQL.
