...
describe('Phase 315 deployment execution retry checkpoint doc', () => {
  it('documents blocked retry scope, commit, and NOT DEPLOYED conclusion', async () => {
    const source = await readFile(join(process.cwd(), PHASE_315_DOC), 'utf8');
    const readme = await readFile(join(process.cwd(), 'README.md'), 'utf8');

    expect(source).toContain('Phase 315');
    expect(source).toContain('Deployment Execution Retry');
    expect(source).toContain('902dae7');
    expect(source).toContain(PHASE_314_DOC.replace('docs/', './'));
    ...
    expect(source).toContain('Phase 315 deployment blockers');
  ...
  it('records missing credentials and blocked deployment steps without secret values', async () => {
    ...
    for (const token of [
      ...
      '2950 passed / 14 skipped',
      'homepage hydrates mixed-feed cards',
    ]) {
      expect(source, `Phase 315 doc should mention ${token}`).toContain(token);
    }
    ...
    for (const section of [
      ...
      expect(source, `Phase 315 doc should contain section ${section}`).toContain(
        section,
      );
    }
  ...
  it('records the validation gates run for the checkpoint', async () => {
    ...
    for (const gate of [
      ...
      expect(source, `Phase 315 doc should record ${gate}`).toContain(gate);
    }
    ...
    expect(source).toContain(
      'phase-315-deployment-execution-retry-checkpoint-doc.test.ts',
    );
    expect(source).toContain(
      'phase-315-deployment-execution-retry-checkpoint.test.ts',
    );
  });
});
