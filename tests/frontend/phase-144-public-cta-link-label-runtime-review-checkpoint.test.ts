...
describe('Phase 144 public CTA / link label runtime review checkpoint', () => {
  it('keeps PUBLIC_CTA_LINK_LABELS and PUBLIC_CTA_* on fixed frontend safe copy only', async () => {
  ...
  it('maps reviewed surface CTA constants into PUBLIC_CTA_LINK_LABELS', async () => {
    const publicUi = await loadModule('public/frontend/public-mvp-ui.js');
  ...
  it('keeps auth / nav CTA labels without user_id, session id, token, or raw auth payload', async () => {
    const authCopy = await loadModule('public/frontend/auth-state-copy.js');
    const publicUi = await loadModule('public/frontend/public-mvp-ui.js');
    ...
    expect(publicUi.PUBLIC_CTA_SIGN_IN_LABEL).not.toMatch(FORBIDDEN_INTERNAL_COPY);
    expect(publicUi.PUBLIC_CTA_REGISTER_LABEL).not.toMatch(FORBIDDEN_INTERNAL_COPY);
  });
  ...
  it('keeps explore, my-polls, create-poll, and lifecycle CTAs without creator token or internal id', async () => {
    const createPoll = await loadModule('public/frontend/create-poll-page.js');
    const creatorFlow = await loadModule('public/frontend/creator-flow-copy.js');
    const lifecycle = await loadModule('public/frontend/poll-lifecycle-controls.js');
    const explore = await loadModule('public/frontend/explore-page.js');
  ...
  it('keeps registration CTA boundary off auto-login, Set-Cookie, and GET /users/me', async () => {
    const registration = await loadModule('public/frontend/registration-page.js');
  ...
  it('does not add observability hooks to reviewed CTA surfaces', async () => {
    for (const relativePath of REVIEWED_CTA_SURFACES) {
      const source = stripJsComments(
  ...
  for (const relativePath of REVIEWED_CTA_SURFACES) {
    it(`keeps reviewed CTA / link label runtime neutral in ${relativePath}`, async () => {
