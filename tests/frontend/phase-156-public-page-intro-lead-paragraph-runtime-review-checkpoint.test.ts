...
describe('Phase 156 public page intro / lead paragraph runtime review checkpoint', () => {
  ...
  it('keeps static HTML shells aligned with PUBLIC_* lead constants', async () => {
    ...
    expect(resultsHtml).toContain(publicUi.PUBLIC_RESULTS_PAGE_DEMO_INTRO_LEAD);
    expect(myPollsHtml).toContain(publicUi.PUBLIC_MY_POLLS_PAGE_LEAD);
    expect(createPollHtml).toContain(publicUi.PUBLIC_CREATE_POLL_PAGE_LEAD);
    ...
    expect(loginHtml).toContain(publicUi.PUBLIC_LOGIN_PAGE_LEAD_SECONDARY);
    expect(registrationHtml).toContain(publicUi.PUBLIC_REGISTRATION_PAGE_LEAD_PRIMARY);
    expect(registrationHtml).toContain(publicUi.PUBLIC_REGISTRATION_PAGE_LEAD_SECONDARY);
