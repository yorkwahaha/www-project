...
describe('Phase 190-R high-quality poll badge minimal frontend rendering runtime review checkpoint', () => {
  it('documents Phase 190 runtime review and Phase 191 plan-only approval', async () => {
    const doc = await readFile(join(process.cwd(), PHASE_190R_DOC), 'utf8');

    expect(doc).toContain('Phase 190-R');
    expect(doc).toContain('Phase 190');
    expect(doc).toContain('APPROVED — Phase 191 blockers: none identified.');
  ...
  it('confirms explore rendering exists; poll detail and results badge rendering deferred to Phase 192+', async () => {
