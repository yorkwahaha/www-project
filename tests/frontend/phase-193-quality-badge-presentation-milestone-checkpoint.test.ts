...
describe('Phase 193 quality badge presentation milestone checkpoint', () => {
  it('confirms Phase 193 is docs/static only with README index reference', async () => {
    const doc = await readFile(join(process.cwd(), PHASE_193_DOC), 'utf8');
    const readme = await readFile(join(process.cwd(), 'README.md'), 'utf8');

    expect(doc).toContain('Phase 193');
    ...
    expect(doc).toContain('modify `src/` runtime');
    expect(doc).toContain('`public/frontend/` behavior');
