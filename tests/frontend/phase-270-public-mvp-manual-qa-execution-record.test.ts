...
const PHASE_270_MARKER = 'Phase 270';
...
describe('Phase 270 public MVP manual QA execution record', () => {
  it('confirms Phase 268/269 docs and Phase 270 execution record exist with README index', async () => {
    ...
    expect(record).toContain('Phase 270');
    expect(record).toContain('Manual QA pass recorded');
    expect(record).toContain('not launch approval');
    expect(record).toContain('PASS');
    expect(record).toContain('Route/Flow');
  ...
  it('keeps Phase 270 as docs-only without runtime, HTML, CSS, or JS delivery markers', async () => {
    for (const relativePath of PUBLIC_HTML_SHELLS) {
      const source = await readFile(join(process.cwd(), relativePath), 'utf8');
      expect(source, relativePath).not.toContain(PHASE_270_MARKER);
    }
  ...
  it('keeps protected backend paths and migrations free of Phase 270 markers', async () => {
    for (const relativePath of PROTECTED_BACKEND_PATHS) {
      const source = await readFile(join(process.cwd(), relativePath), 'utf8');
      expect(source, `${relativePath} must not contain ${PHASE_270_MARKER}`).not.toContain(
        PHASE_270_MARKER,
      );
    }
    ...
    for (const fileName of migrationFiles) {
      ...
      expect(source, `migrations/${fileName} must not contain ${PHASE_270_MARKER}`).not.toContain(
        PHASE_270_MARKER,
      );
    }
  });

  it('locks QA-record boundary checks without runtime changes', async () => {
    const record = await readFile(join(process.cwd(), PHASE_270_DOC), 'utf8');
  ...
  it('keeps vote-by-index payload as { option_index } only during QA record phase', async () => {
  ...
  it('keeps registration and quality_badge boundaries unchanged during QA record phase', async () => {
    ...
    await registration.submitRegistrationRequest({
      profile: {
        display_name: 'QA Record User',
        birth_year_month: '1988-12',
