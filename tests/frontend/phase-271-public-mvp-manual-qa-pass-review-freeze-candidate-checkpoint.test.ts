...
const PHASE_271_MARKER = 'Phase 271';
...
describe('Phase 271 public MVP manual QA pass review freeze candidate checkpoint', () => {
  it('confirms Phase 268-270 docs and Phase 271 review checkpoint exist with README index', async () => {
    ...
    const review = await readFile(join(process.cwd(), PHASE_271_DOC), 'utf8');
    ...
    expect(record).toContain('no runtime bug fixes in this phase');
    expect(review).toContain('Phase 271');
    ...
    expect(review).toContain('not production approval');
    expect(review).toContain('ready for manual QA / freeze candidate');
    expect(review).toContain('separate numbered phase');
    expect(review).toContain('separate explicit phase');
    expect(readme).toContain('Phase 271');
    expect(readme).toContain(PHASE_271_DOC);
  });

  it('keeps Phase 271 as docs-only without runtime, HTML, CSS, or JS delivery markers', async () => {
    for (const relativePath of PUBLIC_HTML_SHELLS) {
      const source = await readFile(join(process.cwd(), relativePath), 'utf8');
      expect(source, relativePath).not.toContain(PHASE_271_MARKER);
    }
  ...
  it('keeps protected backend paths and migrations free of Phase 271 markers', async () => {
    for (const relativePath of PROTECTED_BACKEND_PATHS) {
      const source = await readFile(join(process.cwd(), relativePath), 'utf8');
      expect(source, `${relativePath} must not contain ${PHASE_271_MARKER}`).not.toContain(
        PHASE_271_MARKER,
      );
    }
    ...
    for (const fileName of migrationFiles) {
      ...
      expect(source, `migrations/${fileName} must not contain ${PHASE_271_MARKER}`).not.toContain(
        PHASE_271_MARKER,
      );
    }
  });

  it('locks freeze-candidate review boundary checks documented in the checkpoint', async () => {
    const review = await readFile(join(process.cwd(), PHASE_271_DOC), 'utf8');
    ...
    expect(review).toContain('no runtime bug fixes in Phase 270');
    expect(review).toContain('Launch approval requires a separate explicit phase');
  ...
  it('keeps registration and quality_badge boundaries unchanged during review checkpoint phase', async () => {
    ...
    await registration.submitRegistrationRequest({
      profile: {
        display_name: 'Freeze Candidate User',
