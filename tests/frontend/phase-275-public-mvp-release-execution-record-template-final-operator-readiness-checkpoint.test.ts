...
const PHASE_275_MARKER = 'Phase 275';
...
describe('Phase 275 public MVP release execution record template final operator readiness checkpoint', () => {
  it('confirms Phase 274 handoff and Phase 275 checkpoint exist with README index', async () => {
    ...
    expect(checkpoint).toContain('Phase 275');
    ...
    expect(checkpoint).toContain('9755810');
  ...
  it('keeps Phase 275 as docs-only without runtime, HTML, CSS, or JS delivery markers', async () => {
    for (const relativePath of PUBLIC_HTML_SHELLS) {
      const source = await readFile(join(process.cwd(), relativePath), 'utf8');
      expect(source, relativePath).not.toContain(PHASE_275_MARKER);
    }
  ...
  it('keeps protected backend paths and migrations free of Phase 275 markers', async () => {
    for (const relativePath of PROTECTED_BACKEND_PATHS) {
      const source = await readFile(join(process.cwd(), relativePath), 'utf8');
      expect(source, `${relativePath} must not contain ${PHASE_275_MARKER}`).not.toContain(
        PHASE_275_MARKER,
      );
    }
    ...
    for (const fileName of migrationFiles) {
      ...
      expect(source, `migrations/${fileName} must not contain ${PHASE_275_MARKER}`).not.toContain(
        PHASE_275_MARKER,
      );
    }
  });

  it('locks template checkpoint boundary checks without runtime or deploy changes', async () => {
    const checkpoint = await readFile(join(process.cwd(), PHASE_275_DOC), 'utf8');
    ...
    expect(checkpoint).toContain('Post-Release Smoke Result Template');
    expect(homeSource).not.toContain('creator_session');
    expect(homeSource).not.toContain('X-User-Id');
  });

  it('keeps vote-by-index payload as { option_index } only during template checkpoint phase', async () => {
  ...
  it('keeps registration and quality_badge boundaries unchanged during template checkpoint phase', async () => {
    ...
    await registration.submitRegistrationRequest({
      profile: {
        display_name: 'Template Checkpoint User',
