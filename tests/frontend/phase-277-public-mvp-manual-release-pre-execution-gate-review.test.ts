...
const PHASE_277_MARKER = 'Phase 277';
...
describe('Phase 277 public MVP manual release pre-execution gate review', () => {
  it('confirms Phase 276 record and Phase 277 gate review exist with README index', async () => {
    ...
    expect(review).toContain('Phase 277');
    expect(review).toContain('pre-execution gate review');
    expect(review).toContain('does not perform deployment');
    expect(review).toContain('does not add deploy scripts');
    expect(review).toContain('does not modify production configuration');
    expect(review).toContain('pre-release gates');
    expect(review).toContain('separately executed and recorded');
    expect(review).toContain('separate numbered phase');
    expect(review).toContain('e84b643');
  ...
  it('keeps Phase 277 as docs-only without runtime, HTML, CSS, or JS delivery markers', async () => {
    for (const relativePath of PUBLIC_HTML_SHELLS) {
      const source = await readFile(join(process.cwd(), relativePath), 'utf8');
      expect(source, relativePath).not.toContain(PHASE_277_MARKER);
    }
  ...
  it('keeps protected backend paths and migrations free of Phase 277 markers', async () => {
    for (const relativePath of PROTECTED_BACKEND_PATHS) {
      const source = await readFile(join(process.cwd(), relativePath), 'utf8');
      expect(source, `${relativePath} must not contain ${PHASE_277_MARKER}`).not.toContain(
        PHASE_277_MARKER,
      );
    }
    ...
    for (const fileName of migrationFiles) {
      ...
      expect(source, `migrations/${fileName} must not contain ${PHASE_277_MARKER}`).not.toContain(
        PHASE_277_MARKER,
      );
    }
  });

  it('locks pre-execution gate review boundary checks without runtime or deploy changes', async () => {
    const review = await readFile(join(process.cwd(), PHASE_277_DOC), 'utf8');
    ...
    expect(review).toContain('no deploy scripts');
    expect(review).toContain('no deployment performed');
    expect(review).toContain('No deployment has been performed');
    expect(homeSource).not.toContain('creator_session');
    expect(homeSource).not.toContain('X-User-Id');
  });

  it('keeps vote-by-index payload as { option_index } only during gate review phase', async () => {
  ...
  it('keeps registration and quality_badge boundaries unchanged during gate review phase', async () => {
